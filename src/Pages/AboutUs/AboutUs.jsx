    ...
    return (
        <div className="relative min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 overflow-hidden">
            <div className="absolute inset-0 z-0">
                <Particles particleColors={["#6366f1", "#a855f7", "#ec4899"]} particleCount={50} speed={0.1} particleBaseSize={30} />
            </div>
            ...
            <div className="relative z-10 container mx-auto px-4 py-16 sm:py-24">
                <motion.div className="text-center mb-20" initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, type: "spring" }}>
                    <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight bg-clip-text text-transparent bg-linear-to-r from-indigo-500 via-purple-500 to-pink-500">About ContestHub</h1>
                    ...
                    <motion.div variants={itemVariants} className="grid grid-cols-1 gap-6">
                        {features.map((feature, index) => (
                            <div key={index} className="flex items-start space-x-4">
                                <div className="shrink-0 bg-linear-to-br from-purple-600 to-pink-600 p-3 rounded-full">{feature.icon}</div>
                                <div>
                ...
                <motion.div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8" variants={containerVariants} initial="hidden" animate="visible">
                    {teamMembers.map((member, index) => (
                        <motion.div key={index} variants={itemVariants} className="text-center p-6 bg-white dark:bg-gray-800/50 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300">
                            <div className="w-24 h-24 rounded-full bg-linear-to-br from-purple-200 to-pink-200 dark:from-purple-900 dark:to-pink-900 mx-auto mb-4 flex items-center justify-center">
                                <Users className="w-12 h-12 text-purple-600 dark:text-purple-300" />
                            </div>
                            <h3 className="text-xl font-bold">{member.name}</h3>
                            <p className="text-purple-500 dark:text-purple-400">{member.role}</p>
                        </motion.div>
                    ))}
                </motion.div>
                ...
                <motion.div className="text-center mt-24" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 1 }}>
                    ...
                    <a
                        href="/contest"
                        className="inline-block bg-linear-to-r from-purple-600 to-pink-600 text-white font-semibold px-8 py-4 rounded-xl shadow-lg hover:scale-105 transform transition-transform duration-300"
                    >
                        Explore Contests
                    </a>
                </motion.div>
